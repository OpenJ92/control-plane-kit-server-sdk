...
public [installer](fastapi.py.md). It captures the shared prepared receiving value, including sorted installed variable names, target/declaration and the static verifier. Status is
...
[_http_framing.py](_http_framing.py.md). One explicit
...
SDK #26 moves synchronous static interpretation to `_control_dispatch` and byte framing to `_http_framing`. The adapter owns only stream/response handling, one off-loop call and route construction over the prepared context. It does not resnapshot descriptors or recreate replay.
