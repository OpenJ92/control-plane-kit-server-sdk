...
There is no HTTP server, stream reader, path normalizer, timeout or framework import here. ASGI stream collection and response creation stay in the FastAPI adapters; stdlib request-target, body and connection framing now lives in [_stdlib_control.py](_stdlib_control.py.md), which reuses these byte helpers. The verification-only installed probe imports this owner transitively through shared dispatch while rejecting FastAPI/Starlette/AnyIO.
