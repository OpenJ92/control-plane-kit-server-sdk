...
This private builder consumes the already validated neutral dispatcher and creates the exact Core b79a02d1 health GET template. It is composed only through the existing public installer. Raw method/path must identify exactly liveness/readiness; the query is empty, headers use shared bounded single-Bearer extraction, and streaming rejects the first nonempty body chunk. The adapter does not collect a body or trust forwarded locality. Header/path/query limits retain their existing ownership; elapsed stream time and empty-chunk count are not bounded here.
